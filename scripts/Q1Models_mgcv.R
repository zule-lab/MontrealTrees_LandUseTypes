...
str(subsite_table$Plot)
subsite_table$Plot <- as.factor(subsite_table$Plot)

# Tree Density ------------------------------------------------------------

#Generalized additive model to test the effect of green space type on tree abundance
modelgst_den <- gam(tree_density_plant_ha ~ GreenSpace + s(Plot, bs = "re"),
                     data = subsite_table,
                     family = nb())
summary(modelgst_den) #Random effect > 0, so keep it

#Diagnostic plots
plot.gam(modelgst_den, pages = 1)
gam.check(modelgst_den)
#Not looking too good - but negative binomial is what I was using before in glmer.nb, so what should I do?

#check effect of green space type on tree density using model outputs
anova_den <- anova(modelgst_den, test = "Chisq")
print(anova_result)
